...
Auto-generated from `internal/server/server.go`. Total: **118 tools**.
...
- `agent_suggest` — Given OCR screen text, predict the best next command based on past successful sequences. Returns ranked predictions with confidence scores.
...
<!--
Generated by scripts/gen-tools-doc.go — 118 tools found
-->

---

<sub><sup>
118 tools. auto-generated from server.go. the comment says "118 tools found" as if we're proud of this number. and we are. but also, what does an AI agent do with 118 tools? the first 20 it tries will fail because the ONNX model doesn't work, the next 30 will return "ok" without doing anything, and the last 68 will never be called because nobody scrolls that far. the "System" category alone has 25 tools — more than some entire MCP servers. tool bloat is real, and we're its enablers.
</sup></sub>
