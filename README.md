...
>
> **Status:** v0.2.38 — desktop awareness tools including window Z-order, UIA element tree dump, element-at-point reverse lookup, window-targeted OCR (`ocr_window`, `ocr_active_window`), window bounding rects for screen cross-referencing, plus statistical prior model, training pipeline, memory-backed UI element cache, ONNX detection, runtime privacy controls, key hold/release, input recording, set_config, YOLO dataset export, introspection engine, adaptive ML engine, OCR→command training bridge, ONNX cascade fallback for template matching, per-tool enable/disable via `tool_denylist`, auto-retention pruning via `retention_days`, and UIA-integrated chain step types (`verify_ui`, `if_uia`) with auto-capture of element-at-point after mouse actions. See [`docs/reference/tools.md`](docs/reference/tools.md) for the full listing.
